---
title: "Using rvest to scrape a website"
author: "Tora Mullings"
date: "3/27/2022"
output: html_document
---

The tidyverse includes rvest but it is not loaded when `library(tidyverse)` is run.
...
### Load the dataset, originally found on Kaggle at https://www.kaggle.com/datasets/matwario/wikipedia-music-links.
```{r}
filepath <- "https://raw.githubusercontent.com/djunga/tidyverse-recipe/main/matrix.csv"

df <- read.csv(filepath, header=TRUE)
glimpse(df)
```

### Take a look at the wikipedia categories in `ARTIST CATEGORY`. Each one is part of the wikipedia url that leads to the actual website. Visit wikipedia itself to get the base url that you can append one of these categories to. For example, `https://en.wikipedia.org/wiki/Category:Musical_groups_established_in_1984`
```{r}
